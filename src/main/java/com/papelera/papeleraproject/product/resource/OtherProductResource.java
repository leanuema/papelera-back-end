...
public class OtherProductResource implements OtherProductEndPoint {
    ...
    @Override
    @GetMapping(value = OtherProductEndPoint.SEARCH_PRODUCT,
    produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.FOUND)
    public List<OtherProductDTO> searchProduct(@ModelAttribute("search") OtherProductDTO otherProductDTO,
                                               ExampleMatcher exampleMatcher) throws Exception {
        exampleMatcher = ExampleMatcher.matching().withMatcher("description",
                ExampleMatcher.GenericPropertyMatchers.contains());
        return otherProductService.searchProduct(otherProductDTO, exampleMatcher);
    }
}
