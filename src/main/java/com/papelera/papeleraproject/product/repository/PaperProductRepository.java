...
public interface PaperProductRepository extends JpaRepository<PaperProductModel, Long> {
    ...
    List<PaperProductModel> findProductByStatusId(@Param("statusId") Integer statusId);

    @Query(value = "SELECT cp.PRODUCT_ID, cp.PRODUCT_NAME, cp.PRODUCT_QUANTITY_PRICE, cp.PRODUCT_UNIT_PRICE, cp.STATUS_ID, cp.FEATURED_ID " +
            "FROM PAPER_PRODUCT cp WHERE cp.STATUS_ID = :featuredId", nativeQuery = true)
    List<PaperProductModel> findProductByFeaturedStatusId(@Param("featuredId") Long statusId);
}
